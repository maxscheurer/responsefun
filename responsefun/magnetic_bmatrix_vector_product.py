...
def bmvp_adc2(ground_state, dip, vec):
    ...
    ph = (
            ...
            - 1.0 * einsum('ik,ka->ia', dip.oo, vec.ph)
            - 1.0 * einsum('ic,ja,cj->ia', vec.ph, p0.ov, dip_vo)
            - 1.0 * einsum('ic,jc,aj->ia', vec.ph, p0.ov, dip_vo)
            - 1.0 * einsum('ka,ib,bk->ia', vec.ph, p0.ov, dip_vo)
    ...
    pphh = (
            ...
            + 0.5 * (
                (
                    ...
                    + 1.0 * einsum('ia,jnbf,fn->ijab', vec.ph, t2, dip_vo)
                    + 1.0 * einsum('ja,ib->ijab', vec.ph, dip.ov)
                    - 1.0 * einsum('ja,inbf,fn->ijab', vec.ph, t2, dip_vo)
                    + 1.0 * einsum('ib,ja->ijab', vec.ph, dip.ov)
                    - 1.0 * einsum('ib,jnaf,fn->ijab', vec.ph, t2, dip_vo)
                    - 1.0 * einsum('jb,ia->ijab', vec.ph, dip.ov)
                    + 1.0 * einsum('jb,inaf,fn->ijab', vec.ph, t2, dip_vo)
                ).antisymmetrise(0,1).antisymmetrise(2,3)
                +(
                    - 1.0 * einsum('ka,ijeb,ek->ijab', vec.ph, t2, dip_vo)
                    + 1.0 * einsum('kb,ijea,ek->ijab', vec.ph, t2, dip_vo)
                ).antisymmetrise(2,3)
                +(
                    - 1.0 * einsum('ic,njab,cn->ijab', vec.ph, t2, dip_vo)
                    + 1.0 * einsum('jc,niab,cn->ijab', vec.ph, t2, dip_vo)
                ).antisymmetrise(0,1)
            )
...
if __name__ == "__main__":
    ...
    # magnetic
    # TODO: make it work (not equal yet)
    product_vecs_mag = b_matrix_vector_product(method, mp, magdips, state.excitation_vector)
    ...
    omega_o = 1.0
    omega_2 = 0.5
    rvecs1 = [solve_response(matrix, RV(rhs), omega_2, gamma=0.01) for rhs in mtms]
    components1 = list(product([0, 1, 2], repeat=2))
    product_bmatrix_rvecs1 = b_matrix_vector_product_complex(method, mp, dips, rvecs1)
    beta_tens1 = np.zeros((3,3,3), dtype=complex)
    for c in components1:
        rhs = product_bmatrix_rvecs1[c]
        rvec2 = solve_response(matrix, rhs, omega_o, gamma=0.02)
        for A in range(3):
            beta_tens1[A][c] = mtms[A] @ rvec2.real + 1j* (mtms[A] @ rvec2.imag)
    
    beta_sos_term = (
            TransitionMoment(O, op_a, n) * TransitionMoment(n, op_b, k) * TransitionMoment(k, op_c, O) / ((w_n - w_o - 1j*gamma) * (w_k - w_2 - 1j*gamma))
    )
    omegas_beta = [(w_o, omega_o), (w_2, omega_2)]
    beta_tens1_ref = evaluate_property_isr(state, beta_sos_term, [n, k], omegas_beta, gamma_val=0.01, extra_terms=False)
