...
def bmvp_adc0(ground_state, dip, vec, signs):
...
def bmvp_adc2(ground_state, dip, vec, signs):
    assert type(vec) == AmplitudeVector
    ...
    ph = (
            ...
            - signs[0] * 2.0 * einsum('ic,ac->ia', vec.ph, einsum('ja,jc->ac', p0.ov, dip.ov).symmetrise(0,1))
            #- 1.0 * einsum('ka,ib,kb->ia', vec.ph, p0.ov, dip.ov)
            #- 1.0 * einsum('ka,kb,ib->ia', vec.ph, p0.ov, dip.ov)
            - signs[1] * 2.0 * einsum('ka,ik->ia', vec.ph, einsum('ib,kb->ik', p0.ov, dip.ov).symmetrise(0,1))
            - 0.25 * einsum('ic,mnef,mnaf,ec->ia', vec.ph, t2, t2, dip.vv)
            - 0.25 * einsum('ic,mnef,mncf,ea->ia', vec.ph, t2, t2, dip.vv)
            #- 0.5 * einsum('ic,ac->ia', vec.ph, einsum('mnef,mnaf,ec->ac', t2, t2, dip.vv).symmetrise(0,1))
            ...
            #+ 0.5 * einsum('ka,ik->ia', vec.ph, einsum('mnef,inef,km->ik', t2, t2, dip.oo).symmetrise(0,1))
            ...
            #)
            - signs[2] * 2.0 * einsum('ilad,ld->ia', vec.pphh, dip.ov)
            + signs[3] * 2.0 * einsum('ilad,lndf,nf->ia', vec.pphh, t2, dip.ov)
            - signs[4] * 1.0 * einsum('klad,kled,ie->ia', vec.pphh, t2, dip.ov)
            - signs[5] * 1.0 * einsum('ilcd,nlcd,na->ia', vec.pphh, t2, dip.ov)
    )
    ...
    pphh = (
            ...
            + 0.5 * (
                (
                    - signs[6] * 1.0 * einsum('ia,jb->ijab', vec.ph, dip.ov)
                    + signs[7] * 1.0 * einsum('ia,jnbf,nf->ijab', vec.ph, t2, dip.ov)#
                    + signs[8] * 1.0 * einsum('ja,ib->ijab', vec.ph, dip.ov)
                    - signs[9] * 1.0 * einsum('ja,inbf,nf->ijab', vec.ph, t2, dip.ov)
                    + signs[10] * 1.0 * einsum('ib,ja->ijab', vec.ph, dip.ov)
                    - signs[11] * 1.0 * einsum('ib,jnaf,nf->ijab', vec.ph, t2, dip.ov)#
                    - signs[12] * 1.0 * einsum('jb,ia->ijab', vec.ph, dip.ov)
                    + signs[13] * 1.0 * einsum('jb,inaf,nf->ijab', vec.ph, t2, dip.ov)
                ).antisymmetrise(0,1).antisymmetrise(2,3)
                +(
                    - signs[14] * 1.0 * einsum('ka,ijeb,ke->ijab', vec.ph, t2, dip.ov)#
                    + signs[15] * 1.0 * einsum('kb,ijea,ke->ijab', vec.ph, t2, dip.ov)
                ).antisymmetrise(2,3)
                +(
                    - signs[16] * 1.0 * einsum('ic,njab,nc->ijab', vec.ph, t2, dip.ov)#
                    + signs[17] * 1.0 * einsum('jc,niab,nc->ijab', vec.ph, t2, dip.ov)
                ).antisymmetrise(0,1)
            )
...
def b_matrix_vector_product(method, ground_state, dips, vecs, signs):
    ...
    for c in comp:
        dip = dips[c[0]]
        vec = vecs[c[1:]]
        ret[c] = DISPATCH[method.name](ground_state, dip, vec, signs)
...
if __name__ == "__main__":
    ...
    product_vecs = b_matrix_vector_product(method, mp, dips, state.excitation_vector, signs)
    #print(product_vecs[0][0].pphh.transpose)
    ...
    excitation =  state.excitations[0]
    diffdm = excitation.state_diffdm
    dipmom_ref = [
            product_trace(diffdm, dip) for dip in magdips
    ]
    signs_mag = list(product([1.0, -1.0], repeat=18))
    print(signs_mag)
    for sm in signs_mag:
        print(sm)
        product_vecs_mag = b_matrix_vector_product(method, mp, magdips, excitation.excitation_vector, sm)
        dipmom = [excitation.excitation_vector @ pr for pr in product_vecs_mag]
        if np.allclose(dipmom, dipmom_ref, atol=1e-12):
            print(sm)
            break
